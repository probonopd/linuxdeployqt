...
export ARCH=`arch`
if [ "$ARCH" == "amd64" ]; then
  export PPA=ppa:beineri/opt-qt593-trusty
fi
if [ "$ARCH" == "arm64" ]; then
  export PPA=ppa:adix-me/opt-qt594-xenial-arm64
fi
...
if [ "$ARCH" == "amd64" ]; then
  wget https://ftp.fau.de/debian/pool/main/p/patchelf/patchelf_0.8-2_$ARCH.deb
  echo "5d506507df7c02766ae6c3ca0d15b4234f4cb79a80799190ded9d3ca0ac28c0c  patchelf_0.8-2_$ARCH.deb" | sha256sum -c
 sudo dpkg -i patchelf_0.8-2_$ARCH.deb
